      ...
      {filteredBlogs.length === 0 ? (
        ...
        <div className="grid gap-4 grid-cols-1">
          {filteredBlogs.map((blog) => (
            <div key={blog._id} className="bg-white border-gray-300 border-2 p-4 rounded-lg relative">
              <h2 className="text-lg font-bold pb-3 border-b-2">
                {blog.title}
              </h2>
              <img src={blog.thumbnail} alt={blog.title} className="w-full h-auto pb-3" />
              <p className="py-3 border-b-2"><strong>Author:</strong> {blog.author}</p>
              <h3 className="text-lg font-bold p-3 bg-gray-200">{blog.excerpt}</h3>
